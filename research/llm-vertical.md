...
is finished, five of the six dense families and now the largest MoE row are at
a width the checkpoint does not ship, the one correctness cliff is closed, and
a decode token is attributed to the dispatch. Decode is 36.19 tok/s, 1.44x
llama.cpp, at +1.74% of perplexity, and prefill gains 1.5% beside it.**
**P5's design pass** then measured the thing every estimate of speculation
rested on and had never been run: a **verification pass over M rows costs
3.24x a decode step at M = 2 and 4.32x at M = 8**, so MTP is a net *loss* at
every M and every acceptance rate on the kernels that exist. The reason is
D15 one row along — every block but the MoE takes **one step up at M = 2 and
is then flat to M = 8 on identical weight bytes**, which is a row block that
is 16 to 64 rows wide with two real rows in it — while the MoE's half is real
and is the routing: **10 experts at one token, 17 at two, 36 at four, 49 at
eight**. The rollback the review treated as the hard part is **120.75 MB and
two mechanisms that cost nothing**: the recurrent state ping-pongs free
because the scan loads and stores it exactly once, and the convolution rings
are *deferred* rather than copied — with a correctness finding beside them,
that **both rings are silently corrupted by a single rejected token**. So P5
...
**speculation is lossless and runs at 0.95x on the shipped banks**: 34.65
against 36.38 tok/s short and 28.20 against 31.56 long, two runs each,
token-for-token identical text at temperature zero. The gap to the projected
1.33x is three numbers and none of them is the rollback: the verification pass
is **1.36 decode steps rather than P5b's 24-layer 1.17**, acceptance on the
loop's own traffic is **65.6% on prose and 46.9% at a long context rather than
74.0%**, and **at R = 2 a rejection costs a whole extra pass** — its re-run prefix fills the pass and
displaces the draft, which the design priced as free at general M. Getting
there cost three defects in P5b, two of which made *every* two-row pass
silently wrong (`PinSchedule` stopped pinning at two rows; the four MoE expert
dispatches never named their R-row pipeline) and one of which cost 9% of a
step (the R-row GEMV re-read the bank per row — "it is in the L1 after the
first pass" is true on a two-layer fixture and false across 48 layers of
expert bank). And **453 MB of host memset a pass** in `DeltaNetGPU.Resize`,
which a prefill cannot see because `nTok == arenaRows` and a pre-recorded
decode step cannot see because it resizes once rather than per layer. Two
rungs are now in `TestGraphIsAChunkSplit` — **two tokens at a time and three**
— because a two-row schedule was exercised by no gate in the suite and all
three defects are visible there. **And narrowing the trunk makes speculation
worse, twice**: the draft head stages at the checkpoint's own widths and so
does not shrink with D19/D20/D21, which takes it from 0.176 of a step to
0.208, and acceptance at a long context falls 60.9% → 46.9% because the draft
predicts the checkpoint's next token and the shipped banks move the trunk
1.74% of perplexity away from it. Break-even acceptance is **~0.72**. **The loop is parked rather than shipped
and costs nothing off**: the rollback's 120.75 MB is `GraphOpts.Speculative`,
...
out *faster* at two rows than at one on identical weight bytes. Its lasting
lesson is an instrument one: the first round of measurements ran the
**previous** SPIR-V, because `.spv` is generated and gitignored and the
`.comp` edits were never followed by `go generate` — and every test passed,
because comparing a new kernel's second output row against the old kernel's
is a comparison over a buffer the old kernel wrote. **A control has to be
able to fail**; the one that works changes the second token and requires the
second output row to move.
**P5a before it**: the draft head runs, and its acceptance rate is **74.0%**
over 1024 rounds at n_ctx 2048 against a zeroed-hidden control at 9.8%, flat
in context and identical count-for-count over two runs. The wiring it had no
oracle for is settled **by measurement** — five readings raced from one
recorded trunk walk, and the wide 10240 residual with `hnorm` as `[2560, 4]`
and `eh_proj` applied **once per hyper-connection stream** wins at 73.4%
against the collapsed-broadcast reading's 55.5%, while flipping the
concatenation scores **0 of 256**. What it is worth is smaller than the
carried figure: a round of depth M verifies in **M+1 rows**, so the optimum is
**depth 1 at ~1.34x** where before P5b every depth was a **loss** (0.52x) —
and because a depth-1 verify pass is two rows, P5b ships at **R = 2**, which
...
freed against 0.098, and a perplexity cost the instrument cannot resolve
against one it can). Half of that stage was the kernel: in ggml's eighteen-byte
record the 4.5-bit arm read **152.7 GB/s against Q4_1's 211.4** and was slower
than the six-bit format it replaced, and what fixed it was noticing that **a
transcoded tensor's byte layout is not the checkpoint's** — and then that the
two kernels want different things from it, a plane of scales being 1.13x at
decode and 0.74x at prefill where **36-byte pairs are 204.3 GB/s and cost
prefill nothing** (D21). **P0** found that the
...
speculation is built, lossless, and **0.95x**. The design pass's one blocking
measurement — the M = 2-8 verification pass — said it could not be built on
the kernels that existed; P5b built the kernels, P5c built the loop, and what
the loop says is that a two-row pass is 1.32 decode steps, a 64% draft head
does not pay for it, and at R = 2 a rejection costs a whole extra pass.
**P6 is the only open item on the list.** P1b was the largest item on the list by a factor of four and
...
Two things worth reading twice, **and P4 found both readings wrong.**

**`ffn_down_exps` is Q5_1 on 43 of 48 layers and Q8_0 on the other 5** while
gate/up are Q4_K — read here as "unsloth's imatrix telling them the down
projection is the sensitive one", 27 GB of the 77. It is not. `ffn_down_exps`
is `[640, 2560, 512]` and a ggml K-quant super-block is **256 elements**, so
the row cannot be a K-quant at all; `llama-quantize`'s `tensor_type_fallback`
demotes `Q5_K → Q5_1` and `Q6_K → Q8_0` for exactly this, which is the two
types on the table. It is a fact about the **row length**, and the difference
matters: the sensitivity reading says narrowing `down` would be expensive,
the format reading says **nobody has measured it** (P4c).

And **the routers are F32** — 0.25 GB read every single token, 4% of the
decode budget for 0.06 B of parameters. In the *checkpoint*. `MoEGPU.stage`
...
- [x] ~~Then the two the simulation cannot reach~~ *(P4, done 2026-09-19 —
      **and neither was what it said**)*. ~~**the router to fp16**
      (+1.5 tok/s of ceiling, D3's own line, and L5a's ties are the risk)~~ —
      it has been fp16 since **L5b**, the row is 0.142 GB a token and not
      0.252, and L5a's ties were priced by every perplexity number in this
      log. ~~and **the 512 expert banks at ~4.25** (+3.0 tok/s, D4's floor)~~
      — `ffn_down_exps` has rows of **640** and a K-quant super-block is
      256, so Q4_K does not exist for it; the width is `llama-quantize`'s
      fallback, not the imatrix's judgement. **D20** took the four rows a
      transcode can reach instead, and the real question is **P4c**.
      Everything
...
- [x] MTP speculative decoding (the separate GGUF). Target was 1.5-1.8x.
      *(P5 — **done 2026-09-19, and the answer is 0.95x.** `cmd/llm -spec`:
      ...
      round. It is **lossless** — the plain loop's ids token for token at
      temperature zero, at ctx 512 and at 2048 cells — and it is a **net loss
      in rate**, because the pass is 1.32 decode steps, acceptance on real
      generated text is 64.1%, and at R = 2 a rejected round costs a whole
      extra pass. It also has a **hard ceiling at 2048 cells**: `blk.48`
      states no compress ratio, so the draft head refuses any context where
...
## How to run things
    ...
    # P3: the knapsack, measured. Six complete plans over 145 chunks each --
    # never a family alone, because additivity leaks both ways once the
    # n-gram block is in play. A family *omitted* from a plan stages at the
    # checkpoint's own width, which for these Q8_0 projections is exactly
    # L8a's int8 bank, so the int8 arms need no kernel and no new flag.
    Q4K=deltanet=q4_k/32,hyper_conn=q4_k/32,lm_head=q4_k/32,ple_proj=q4_k/32,full_attn=q4_k/32,qsa_indexer=q4_k/32
    LLM_DENSE_SIM_QUANT=imatrix LLM_DENSE_SIM=$Q4K \
        go run ./cmd/llm -ppl -model $M                      # the control, 4.1998
    LLM_DENSE_SIM=...,full_attn=q5_k/32,qsa_indexer=q5_k/32 ...   # 4.1560
    LLM_DENSE_SIM=...,lm_head=q5_k/32 ...                         # 4.1743
    LLM_DENSE_SIM=...,ple_proj=q5_k/32 ...                        # 4.1919
    LLM_DENSE_SIM=deltanet=q4_k/32,hyper_conn=q4_k/32,lm_head=q4_k/32,ple_proj=q4_k/32 \
        ...                                       # full_attn off = int8, 4.1385

    # D18 itself, on the real bank -- ple_proj off the plan. Bank and
    # simulation agree to four decimals and to the same standard error.
    D18=deltanet=q4_k/32,hyper_conn=q4_k/32,lm_head=q4_k/32,full_attn=q4_k/32,qsa_indexer=q4_k/32
    LLM_DENSE_BANK=$D18 go run ./cmd/llm -ppl -model $M \
        -csv results/p3_ppl_d18_bank.csv                     # 4.1850, +3.87%
    LLM_DENSE_BANK=$D18 go run ./cmd/llm -gen -model $M -n 64 \
        -prompt 'The capital of France is'   # interleave A/B/A/B/A/B: the
                                 # within-arm spread is 0.31 tok/s and the
                                 # difference between the two arms is 0.04
    go run ./cmd/serve -llm      # stages D19 by default; LLM_DENSE_BANK
                                 # overrides it, `off` serves the int8 bank

    # P3a: the fifth bit. The gate first -- the same complete plan P3
    # simulated at 4.1560, now on the real bank, and `results/` compared
    # chunk for chunk: 0 of 145 rows differ.
    LLM_DENSE_BANK=deltanet=q4_k/32,hyper_conn=q4_k/32,lm_head=q4_k/32,\
full_attn=q5_k/32,qsa_indexer=q5_k/32,ple_proj=q4_k/32 \
        go run ./cmd/llm -ppl -model $M -csv results/p3a_ppl_attn_q5k.csv
    paste -d, <(cut -d, -f4 results/p3a_ppl_attn_q5k.csv) \
        <(cut -d, -f4 results/p3_ppl_attn_q5k.csv) | awk -F, 'NR>1 && $1!=$2'

    # then the ladder, on D18's basis: one family at a time, in P3's order
    # of estimated return, each a complete plan over 145 chunks.
    LLM_DENSE_BANK=deltanet=q4_k/32,hyper_conn=q4_k/32,lm_head=q4_k/32,\
full_attn=q5_k/32,qsa_indexer=q5_k/32 go run ./cmd/llm -ppl -model $M \
        -csv results/p3a_ppl_d18_attn.csv                    # 4.1386, +2.72%
    # ...,lm_head=q5_k/32,...      -> p3a_ppl_d18_attn_head.csv     4.1136
    # ...,hyper_conn=q5_k/32,...   -> p3a_ppl_d18_attn_head_hc.csv  4.0948 = D19
    # deltanet=q5_k/32,...         -> p3a_ppl_d18_all4.csv          4.0780, refused

    # and the decode ladder: five plans, two interleaved passes, one binary,
    # one hour (P1c's rule). go build once -- `go run` per arm would not be
    # the same binary. 35.70 / 35.49 / 35.11 / 34.57 / 33.13 tok/s.
    go build -o /tmp/llmbin ./cmd/llm
    for pass in 1 2; do for plan in "$D18" "$ATTN" "$HEAD" "$HC" "$ALL4"; do
        LLM_DENSE_BANK=$plan /tmp/llmbin -gen -model $M -n 64 \
            -prompt 'The capital of France is' | grep ^decode
    done; done
    go test ./llm/ -run 'IsTheSim|Q4Gemv|BankQK' -v   # every bank-against-
                                 # simulation gate, both widths, five blocks

    # P3's second corpus (idea 5). The Go standard library, cut to
    # wiki.test.raw's own byte count so the two runs are the same length;
    # /models is gitignored, so this is the regeneration line.
    mkdir -p models/code-corpus
    find $(go env GOROOT)/src -name '*.go' ! -name '*_test.go' \
        ! -path '*/testdata/*' | LC_ALL=C sort | xargs cat \
        | head -c 1290590 | head -n -1 > models/code-corpus/go.test.raw
    go run ./cmd/llm -ppl -model $M -ppl-file models/code-corpus/go.test.raw \
        -chunks 145 -csv results/p3_code_base.csv            # 1.6288
    # and the same with LLM_DENSE_BANK=$Q4K (1.6560), =$D18 (1.6506) and
    # LLM_DENSE_SIM at the full_attn q5_k plan (1.6450)
    ...
    # P4c: the down projection below six bits. Two formats, each a complete
    # plan over 145 chunks on D19's dense bank and D20's MoE plan, with
    # `down_exps` the only thing that changes. `iq4_nl` is D21 and is what
    # `ShippedMoEBank` carries; `q4_1` is the arm it beat.
    D19="deltanet=q4_k/32,hyper_conn=q5_k/32,lm_head=q5_k/32,full_attn=q5_k/32,qsa_indexer=q5_k/32"
    D20=gate_shexp=q4_k,up_shexp=q4_k,down_shexp=q5_1
    LLM_DENSE_BANK=$D19 LLM_MOE_BANK=$D20,down_exps=iq4_nl \
        go run ./cmd/llm -ppl -model $M -csv results/p4c_ppl_iq4_nl.csv   # 4.0992
    LLM_DENSE_BANK=$D19 LLM_MOE_BANK=$D20,down_exps=q4_1 \
        go run ./cmd/llm -ppl -model $M -csv results/p4c_ppl_q4_1.csv     # 4.1092
    # the paired chunk-for-chunk test against D20's own run, which is where
    # the two formats separate: q4_1 is resolvable (t = 4.11) and iq4_nl is
    # not (t = 0.79). The CSVs are cumulative, so a chunk's own nll is a
    # difference of token-weighted sums.
    python3 - results/p4_ppl_all.csv results/p4c_ppl_iq4_nl.csv <<'PY'
    import csv, math, sys
    def perchunk(p):
        rows=list(csv.DictReader(open(p))); out=[]; pt=pn=0.0
        for r in rows:
            t,n=float(r['tokens']),float(r['nll'])
            out.append((n*t-pn*pt)/(t-pt)); pt,pn=t,n
        return out
    a,b=perchunk(sys.argv[1]),perchunk(sys.argv[2])
    d=[y-x for x,y in zip(a,b)]; n=len(d); m=sum(d)/n
    sd=math.sqrt(sum((x-m)**2 for x in d)/(n-1))
    print(f"mean {m:+.6f} t {m/(sd/math.sqrt(n)):.2f} worse {sum(1 for x in d if x>0)}/{n}")
    PY
    # and the decode arms, three passes interleaved over one binary (P1c)
    go build -o /tmp/llmbin ./cmd/llm
    for pass in 1 2 3; do for f in q5_1 q4_1 iq4_nl; do
        LLM_DENSE_BANK=$D19 LLM_MOE_BANK=$D20,down_exps=$f /tmp/llmbin -gen \
            -model $M -n 64 -prompt 'The capital of France is' | grep ^decode
    done; done
    go test ./llm/ -run TestMoEPackersAgainstGGML -v    # the two packers
                                 # against ggml's own quantiser: iq4_nl's
                                 # uncalibrated arm is bit-exact over 40 960
                                 # values, and D10's stored-half assignment
                                 # is measured rather than asserted
    go test ./llm/ -run 'TestMoEGPUDownNarrowed|TestMoEBankTranscode' -v
                                 # the kernels: every rung exact, the two
                                 # unpacks 2.6e-06 apart, and a transcoded
                                 # routed bank bit-identical to a checkpoint
                                 # that shipped the format
    # the oracle for the packers, on dequant_ref.c's precedent (same binary
    # as L8c-2's, its own fixture because the row here is the down
    # projection's 640)
...
- **What does `ffn_down_exps` cost below six bits?** 0.615 GB a token, 41%
  of the expert traffic, and it sits at 6.26 bits only because
  `llama-quantize` had no K-quant to offer a 640-wide row (P4). Every
  candidate blocks by 32 and so fits — IQ4_NL at 4.5 bits, Q4_1 at 5.0,
  Q4_0 at 4.5 — and each needs a `down_` arm at a new `QFMT`. **This is the
  first MoE row where a narrowing could plausibly cost real accuracy**: the
  three P4b took are 8.5-bit rows that nothing chose deliberately, where
  this one is the model's own sensitive projection by every heuristic that
  is *not* the row-length artefact. P4c.
- **Why is a dispatch's time not a function of its bytes, in either
  direction?** P3a's fifth bit cost 17% *less* decode than its bytes
  predicted (0.497 GB priced at −3.1 tok/s, measured −2.57) and P4b's
  transcode bought 11% *less* than its bytes predicted (0.129 GB priced at
  +1.11, measured +0.99). Both are the same size of miss with opposite
  signs, and the rung re-screen accounts for a tenth of the second one. The
  common suspect is P1's 5.5 ms of "shape rather than bytes", which nobody
  has modelled.
